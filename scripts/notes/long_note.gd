...
@export var body_target_size: Vector2 = Vector2(120, 40):
	set(value):
		body_target_size = value
		if is_node_ready():
			_update_body_texture_scale()
@export var tail_target_size: Vector2 = Vector2(120, 120):
	set(value):
...
func _ready() -> void:
	...
	body_target_size.y = max((render_height - tail_target_size.y), 0)
